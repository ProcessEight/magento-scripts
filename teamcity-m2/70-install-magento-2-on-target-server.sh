...
sudo -u administrator find var vendor pub/static pub/media app/etc -type f -exec chmod u+w {} \;
echo "# Force correct permissions on directories"
sudo -u administrator  find var vendor pub/static pub/media app/etc -type d -exec chmod u+w {} \;
echo "# Force correct ownership on files"
sudo -u administrator find var vendor pub/static pub/media app/etc -type f -exec chown %env.MAGENTO2_ENV_CLIUSER%:%env.MAGENTO2_ENV_WEBSERVERGROUP% {} \;
echo "# Force correct ownership on directories"
sudo -u administrator find var vendor pub/static pub/media app/etc -type d -exec chown %env.MAGENTO2_ENV_CLIUSER%:%env.MAGENTO2_ENV_WEBSERVERGROUP% {} \;
echo "# Set the sticky bit to ensure that directories are generated with the right ownership"
sudo -u administrator  find var vendor pub/static pub/media app/etc -type d -exec chmod g+s {} \;
echo "# Set the sticky bit to ensure that files are generated with the right ownership"
sudo -u administrator find var vendor pub/static pub/media app/etc -type f -exec chmod g+s {} \;
